...
| `--panels panels.py` | 这张图的 `CELLS`（面板框）/ `ELEMENTS`（元素框+颜色条件）/ `SPLIT` / `ROTATED`（斜排标签）/ `MANUAL` | 照示例改：`panels.py`（T3-01，平图）/ `panels_qgp.py`（QGP，含 `ROTATED`）；词表写法见 `words_qgp.txt` |
...
## 实测指标 #2 —— 斜排标签（QGP 朝向涨落示意图，1200×865）

第二张图（3D 圆柱 + 火球，大量柔和渐变 + 细网格线）比 T3-01 难得多，正好压边界。
...
| 文字覆盖像素 | 1.77%，但占整图误差 **56%** |
| 体积 | 3.3 MB（R=16）；R=12 → 3.6 MB、MAE 2.65（+10% 体积换 −3.7% 误差，不划算） |

**文字那 56% 不是缺陷**：真 `<text>` 是按 Arial 重排的，亚像素级不可能与原图逐像素重合
（单个粗体字母实测残差 36/255，放大看完全一样）。这是「可编辑性」的定价 ——
要衡量临摹保真度，看**非文字区**那一行。

### 斜排标签 —— `panels.ROTATED`

斜着排的箭头说明（`Low-energy method` 这类）走 `labels.align_rot()`：
把图绕框心**转正** → 挑出文字行带 → 交给已验证的水平 `align()` 三段式。

```python
# panels.py —— 格式 (文字, x0, y0, x1, y1, 角度, 字号px)
# 角度 = SVG rotate() 的角度，逆时针为正（往右上斜是负值）
ROTATED = [
...
- 框只要**大致框住**那段文字即可，角度和字号会按实际墨迹残差重估
  （角度扫描 ±6°、步长 2°，判据是实际残差 `res0 + 0.004·dh`，不是几何启发式）。
- 两条平行标签的框**可以重叠** —— 擦除用「旋转后的四边形」掩码，不会互相擦。
- **拟合不过关的整条不擦**、原样保留成色块（门槛 `res0 ≤ max(0.14, 0.55·ink)` 且 `dh ≤ 5`）。
- 缺字（`⊥` `≳` `⟨⟩`）自动换 `fonts.pick_math()` 的兜底字体（DejaVu Sans）。
- 上下标写 `_{2}` / `^{2}`，会变成 `<tspan>`；**不要写 Unicode `₂`**（Arial 无此字形）。
...
- **细网格线 / 网纹**：1px 暗线压在浅渐变上，四叉树按块均值取样会把线**压淡**
  （非文字区 0.59% 的 >32 像素基本都出在这里）。要根治得再加一层「暗线描边」通道；
  调小 `--R` 救不回来（16→12 实测误差只降 3.7%，体积涨 10%）。
