...
def dh_lim(meta, floor=2.0, frac=0.18):
    """几何验收「渲染墨迹高度必须和原图一致」的容差 —— **必须按字号缩放**。

    ★ 实测坑：原来这是个写死的绝对像素（`dh > 2`）。工作分辨率从 1080 提到
      2160 时字号翻倍、dh 也按比例变大，于是**拟合得更好的标签反而被误杀**：
      实测 "Quantum" / "QGP fireball" 在 2160 下 res0 = 0.009（闸门是 0.14，
      好一个数量级），却因为 dh=4 > 2 被丢掉、退回成色块轮廓。
      按墨迹高取比例后：1080 下（墨高 13）仍是 2.0，与原来一致；
      2160 下（墨高 26）自动放宽到 4.7 —— 同一个相对容差。
    """
    ir = meta.get("irect")
    h = (ir[3] - ir[1]) if ir else 0
    return max(floor, frac * h)
