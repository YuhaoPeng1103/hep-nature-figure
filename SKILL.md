...
**版本 A：模型直接写 SVG（默认推荐）** —— IR → 模型直接写 SVG → 三道门禁 → 返修单。**约束最紧、可复现最好，是默认推荐的路。**

流程：
```
IR（物理约束）→ 模型逐元素写 SVG（先定全局光源）→ repair_brief 返修 → 矢量 + PDF
```

**为什么推荐它**：IR 的几何约束全程在场，可复现性最好。\n\n**它的短板**：观感是「干净的教科书插图」，够不到图像模型的质感。要质感就换 B1。
