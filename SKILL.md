...
**版本 B1：生图当草图** —— IR → 生图简报（草图稿）→ 生图 → check_sketch 检查 → 绘画成矢量。**质感与可控的折中。**

流程：
```
IR → ir_to_genbrief --stage sketch → 生图 → check_sketch（几何约束逐条核）→ 模型重画成 SVG → 门禁
```

**关键设计**：图像模型的产物是「**更好的草图**」，不是成品 ——\n所以绘画那步仍要按 IR 的几何约束**正确执行**，不会把位图的物理错误照抄进来。

（其他路线见同仓库的 `route-a` / `route-b1` / `main` 分支）
