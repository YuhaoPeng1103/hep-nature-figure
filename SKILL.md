...
### 完整流程（Scene Graph 是 A/B 共用的最后一环）
...
① 写 IR（物理层 + 元素 + 构图 + geometry_constraints）
        ↓
   选路线 —— 决定「怎么产生内容」
   ├─ A  模型直接写 SVG                      约束最紧、可复现最好
   ├─ B1 生图当草图 → check_sketch 检查 → 绘画   质感与可控的折中
   └─ B2 B1 + 成品位图 → 临摹                观感最好、两次生图最不可复现
        ↓
② Scene Graph 提取 → 确定性重组   ← ★ 三条路都在这里汇合
   模型看懂内容 → 写成结构化描述（对象/文字/公式/几何/层级/坐标）
   → scene_render.py 重组 → SVG
        ↓
③ 三道门禁 + 返修单 → 交付（SVG 工作稿 + PDF 交付稿）
```

### ★ 第 ② 环：为什么需要 Scene Graph
...
| 让模型看着位图直接重画 | **有理解但不忠实、不确定**：漂移、自己发明、两次不一样 |

**第三条路**：模型只负责「**看懂并写出结构化描述**」，重组交给确定性代码。
...
**★ 可复现性因此成立**：同一份 Scene Graph → **逐字节相同的 SVG**（已实测）。
这正是"对库里已有图片保持极高可复现性"的要求 ——
**不是"同 prompt 两次一样"**。
...
- **默认走 A**；要质感走 B1；**B2 只在 B1 明显不够时上**
- **三条路最终都汇到第 ② 环**（Scene Graph 重组），拿到确定的矢量

> ⚠️ **Scene Graph 不解决"好看"**。它解决的是「忠实 + 可复现」。
> 复杂矢量图的美学上限仍取决于模型看懂的细致程度 + 图元库的表达力。
> 别又变成"以为能自动达到 Nature 级"。
