...
**版本 B2（最新）：生图 + 成品位图** —— 两次生图：先出「更好的草图」检查，再出成品位图，然后临摹成矢量。**观感最好，但可复现最差。**

流程：
```
IR → 草图稿简报 → 生图 → check_sketch 检查 → 成品位图简报 → 生图 → 看懂后重画成 SVG → 门禁
```

**唯一新增的风险**：每次生图非确定，两次就是两次漂移机会；第二次是看着第一次的图再画，可能二次解释。**建议先用它跟 B1 跑一次对比再决定。**
