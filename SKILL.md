...
## 路线 B：图像生成 → 临摹成矢量（2026-09-20 新增）

除了"模型直接写 SVG"，还有第二条路，适合**质感优先**的图：

```
① skill 给约束          python3 scripts/ir_to_genbrief.py ir/xxx.ir.yaml \
                            --style-profile assets/style-profiles.json
② 图像模型按简报出位图    （ChatGPT 里就是 GPT Image；本地可挂 API）
③ 模型看着位图【临摹】    ← ★ 是"看懂再重画"，不是像素描摹
...
**这是方法的差别，不是调参能补的。** `raster_to_vector.py` 只当
"没有模型可用时的兜底"，**不是主路径**。
...
图像模型不知道物理。它会把喷注画反、把非中心碰撞画成中心碰撞、
把光锥斜率画歪。**③ 临摹时必须拿 IR 的 `geometry_constraints` 校正，
不能对着位图照抄** —— 否则"好看但物理错"会从位图传染到矢量。

`ir_to_genbrief.py` 把几何约束单独列成第三节，就是给这一步看的。

### 两条路怎么选

| | 路线 A：模型写 SVG | 路线 B：出位图再临摹 |
|---|---|---|
| 观感 | 教科书插画 | **更接近图像模型的质感** |
| 图层 | 语义图层 | 取决于临摹质量 |
| 文字 | 真 `<text>` | 真 `<text>`（模型重画时写的） |
| 可复现 | 同名 prompt 大致稳定 | **取决于图像模型**，风险更高 |
| 物理 | IR 全程约束 | **位图这一步脱离约束**，临摹时要补回来 |

**默认走 A**（约束更紧、可复现性更好）；**要质感时走 B**，但临摹那步要格外较真。
